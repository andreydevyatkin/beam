...
"""
  ...
  void apply(Project project) {
    ...
    project.gradle.taskGraph.whenReady { graph ->
      ...
      //   getExecutionData().setFrom(project.fileTree(project.buildDir).include("/jacoco/test.exec"))
      //   project.subprojects.each { subproject ->
      //     if(subproject.tasks.withType(JacocoReport)) {
      //       println "report: ${subproject}"
      //       // getAdditionalClassDirs().setFrom(subproject.sourceSets.main.allClass.srcDirs)
      //       getAdditionalSourceDirs().setFrom(subproject.sourceSets.main.allSource.srcDirs)
      //       reports {
      //         xml.required = true
      //         html.required = true
      //       }
      ...
      //           println project.subprojects.size()
                
      //           getAdditionalClassDirs().setFrom(report.allClassDirs)
      //           getAdditionalSourceDirs().setFrom(report.allSourceDirs)
      //           reports {
      //             xml.required = true
      //             html.required = true
      //           }
      //         }
      //       // if (project.subprojects.hasProperty("jacocoTestReport")) {
    ...
    project.ext.applyJavaNature = {
      ...
      // Apply and configure Jacoco in each subproject
      project.subprojects { subproject ->
      ...
      // Configure aggregation task in the root project
      project.tasks.register('jacocoRootReport', JacocoReport) {
          project.subprojects.each { subproject ->
          ...
          getExecutionData().setFrom(project.fileTree("${project.buildDir}/jacoco").include('**/*.exec'))

          // Aggregate class directories from all subprojects
          getClassDirectories().setFrom(project.files(project.subprojects.collect {
              it.sourceSets.main.output
          }))

          // Aggregate source directories from all subprojects
          getSourceDirectories().setFrom(project.files(project.subprojects.collect {
      ...
      //     }
      //   }
      //   // Depending on all test tasks of this project and subprojects
      //   project.tasks.named('jacocoTestReport').configure { task ->
      //     task.dependsOn(project.tasks.withType(Test))
      //     project.subprojects.each { subproject ->
      //       task.dependsOn(subproject.tasks.withType(Test))
      //     }
      //   }
      // }
      ...
      //     project.subprojects.each { subproject ->
      //       classDirectories.setFrom(subproject.files(subproject.files(subproject.sourceSets.main.output).collect {
      //               subproject.fileTree(
      //                       dir: it,
      //                       includes: subproject.hasProperty('jacocoIncludes') ? subproject.property('jacocoIncludes').split(',') as List<String> : configuration.jacocoIncludes,
      //                       excludes: subproject.hasProperty('jacocoExcludes') ? subproject.property('jacocoExcludes').split(',') as List<String> : configuration.jacocoExcludes)
      //       }))
      //       getAdditionalSourceDirs().setFrom(subproject.sourceSets.main.allSource.srcDirs)
      //       getSourceDirectories().setFrom(subproject.sourceSets.main.allSource.srcDirs)
      ...
      //       //   println "report: ${it}"
      //       //   // additionalClassDirs it.allClassDirs
      //       //   getAdditionalSourceDirs().setFrom(it.sourceSets.main.allSource.srcDirs)
      //       // }
      //     }
      //     reports {
      //       xml.required = true
      ...
      // project.jacocoTestReport {
      //   println "jacocoTestReport : ${project}"
      //   // getClassDirectories().setFrom(project.files(getClassDirectories().files.collect {
      //   //     project.fileTree(
      //   //       dir: it,
      //   //       includes: project.hasProperty('jacocoIncludes') ? project.property('jacocoIncludes').split(',') as List<String> : configuration.jacocoIncludes,
      //   //       excludes: project.hasProperty('jacocoExcludes') ? project.property('jacocoExcludes').split(',') as List<String> : configuration.jacocoExcludes)
      ...
      // }
        ...
        //   project.subprojects { subProject ->
        //     println project.subprojects.size()
        //     // subProject.tasks.withType(JacocoReport) { report ->
      ...
      //   println project.subprojects.size()
      //   if (project.subprojects.size() > 0) {
      //     getExecutionData().setFrom(project.subprojects.jacocoTestReport.executionData)
      //   }
      //   reports {
      //     xml.required = true
      //     html.required = true
